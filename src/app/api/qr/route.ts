...
import type { CenterDecoration } from "@/lib/center-decoration";
...
export async function POST(request: NextRequest) {
  ...
  const centerDecoration = body?.centerDecoration as CenterDecoration | undefined;
  const centerText = (body?.centerText as string | undefined)?.trim();
  ...
  try {
    ...
    const { data, error } = await supabase
      .rpc("create_qr_code", {
        ...
        p_color: color ?? null,
        p_name: name ?? null,
        p_center_decoration: centerDecoration ?? null,
        p_center_text: centerText || null,
      })
      .single();
